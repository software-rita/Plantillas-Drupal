...
class PrevNextHelper implements PrevNextHelperInterface {
  ...
  public function getPrevId($entity_id) {
    return $this->database->query("SELECT prev_nid FROM {prev_next_node} WHERE nid = :nid", [':nid' => $entity_id])->fetchField();
  }
  ...
  public function getNextId($entity_id) {
    return $this->database->query("SELECT next_nid FROM {prev_next_node} WHERE nid = :nid", [':nid' => $entity_id])->fetchField();
  }
